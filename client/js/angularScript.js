...
myapp.controller('solarCalc', ['$scope', function($scope, $document) {
    ...
    $scope.solarCalc = function() {
        if($scope.calc.amount && $scope.calc.tenure){
            $scope.noEmi = false;
            $scope.showEmi = true;
            var interest = 11;
            ...
            $scope.solarUnitsPerMonth = Math.floor($scope.solarSystemSize * 4 * 30);
            $scope.totalSolarSystemCost = Math.floor(unitSolarSystemCost * $scope.solarSystemSize);
            $scope.currentBill = $scope.calc.units * $scope.calc.location1;
            $scope.reducedBill = ($scope.calc.units - $scope.solarUnitsPerMonth) * $scope.calc.location1;
            $scope.monthlySavings = $scope.currentBill - $scope.reducedBill;
